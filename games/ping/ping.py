...
class JatekTabla(Frame):
    ...
    def initGame(self):
        ...
        for i in range(12): # (egyenertek egy 12 sor x 12 oszlopos ű
    ...
    def drawGrid(self):
        ...
        for l in range(self.nrow -1): # vizszintes vonalak
            ...
            s +=self.side
        s =self.side
        for c in range(self.ncol -1): # függolees vonalak
    ...
    def clic(self, event):
        ...
        row, col = int(event.y/self.side), int(event.x/self.side)
...
class Ping(Frame):
    ...
    def principe(self):
        ...
        text ="A minden korongnak van egy fehér és egy fekete oldala "\
            ...
            "Ha a gyakorlat 2 x 2-es ráccsal nagyon egyszer nek t nik, "\
            "nehezebbé válik nagyobb méret rácsokkal. S t bizonyos "\
            "méret rácsokkal egyáltalán nem lehet lejátszani.!\n\n "\
            "Irodalom : 'Pour la Science' Aout 20022").pack(padx =10, pady =10)
    
    def aPropos(self):
        "A szerz t és a licenc típusát megadó üzenetablak"
        msg =Toplevel(self)
