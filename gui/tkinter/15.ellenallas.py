from tkinter import *
from math import log10 # 10-es alapú logaritmus
...
class Application():
    ...
    def drawResistor(self):
        ...
        self.line =[] # egy listában fogjuk tárolni a három csíkot
        for x in range(85,150,24):
            self.line.append(self.can.create_rectangle(x,30,x+12,70, fill='black',width=0))

    def changeColours(self):
        """A beírt értéknek megfelel három szín kiíratása"""
        self.v1ch = self.entry.get() # get() metódus egy stringet ad vissza
        try:
        ...
        if err ==1 or v < 10 or v > 1e11 :
            ...
            # az els szignifikáns számjegy el állítása : ő ő
            li[0] = int(v/ordgr) # egész rész
            decim = v/ordgr - li[0] # tizedes rész
            # a második szignifikáns számjegy el állítása : ő
            li[1] = int(decim*10 +.5) # +.5 a korrekt kerekítéshez
            # a 2 szignifikáns számjegyhez hozzáteend nullák száma : ő
            li[2] = logv -1
    ...
    def reportError(self):
        self.entry.configure(bg ='red') # mez hátterének színezése ő
...
# Foprogram : 
f = Application() # alkalmazás objektum létrehozása